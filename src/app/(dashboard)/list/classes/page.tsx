...
const ClassesListPage = () => {
  ...
  const renderRow = (item: Class) => (
    ...
    >
      ...
      <td className="hidden md:table-cell">Grade {item.grade}</td>
      <td className="hidden md:table-cell">
        {item.supervisor?.name || "-"}
      </td>
      <td className="hidden lg:table-cell">{item._count?.students || 0}</td>
      <td>
  ...
  return (
    <div className="bg-white p-2 sm:p-4 rounded-md flex-1 m-2 sm:m-4 mt-0">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 mb-4">
        <h1 className="text-lg sm:text-xl font-semibold">All Classes</h1>
      ...
      {classes.length === 0 ? (
        <div className="text-center py-12 text-gray-500">
          No classes found. {searchQuery && "Try a different search."}
        </div>
      ) : (
      ...
      <div className="text-xs sm:text-sm text-gray-500 text-center mt-2 px-2">
        Showing {classes.length} of {pagination.total} classes (Page{" "}
